...
async function checkTurnEndAsync() {
    ...
    if (!isMoving && turnActive) {
        ...
        if ((currentPlayer === 2 || isSelfPlayTraining || isBackgroundTraining) && lastAIState && lastAIAction) {
            const isPerfectShot = (pottedOwnPieceThisTurn || isGameWon) && hitAnyPuckThisTurn && !isFoul;
            await carromBrain.rememberAndTrain(lastAIState, lastAIAction, isPerfectShot);
        }
...
class CarromBrain {
    ...
    async rememberAndTrain(stateArray, actionTaken, isPerfectShot) {
        if (!this.isInitialized) return;

        if (isPerfectShot) {
            this.memory.push({ state: stateArray, action: actionTaken });
